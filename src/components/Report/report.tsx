...
const ReportPage: React.FC = () => {
  ...
  // --- Fetch total count (uses refs to avoid unstable deps) ---
  const fetchTotalCountIfNeeded = useCallback(
    async (whereConstraints: QueryConstraint[]) => {
      ...
      try {
        ...
        console.warn("Count fetch failed (will estimate):", err);
        // fallback: estimate from cache/page info
        const estimated =
          (page - 1) * PAGE_SIZE +
          (pageCacheRef.current[page]?.length ?? apiRows.length);
        setTotalRows((prev) => Math.max(prev, estimated));
      }
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [buildFiltersKey, page, totalRows] // note: pageCache handled via ref; eslint ignore allowed here
  );
  ...
  const fetchPage = useCallback(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [lastDoc,buildFiltersKey, blDate, coDate, rcvDate, debouncedSearch, sortField, sortDirection, fetchTotalCountIfNeeded]
  ...
  const computedDisplayedRows = useMemo(() => {
    ...
    return filtered;
    // note: we intentionally depend on apiRows and the UI filter inputs (searchText etc.)
  }, [apiRows, searchText, blDate, coDate, rcvDate, sortField, sortDirection]);
              ...
              <TableRow sx={{ bgcolor: "#f5f5f5" }}>
                <TableCell padding="checkbox">
                  <Checkbox
                    ...
                    indeterminate={selectedIds.length > 0 && !displayedRows.every((r) => selectedIds.includes(r.id))}
                  />
                </TableCell>
